...
def c51(seed=1626, eps_test=0.05, eps_train=0.1,
        buffer_size=20000, lr=1e-3, gamma=0.9, num_atoms=51, v_min=-10.0, v_max=10.0,
        n_step=3, target_update_freq=320, epoch=10, step_per_epoch=8000, step_per_collect=8,
        update_per_step=0.125, batch_size=64, hidden_sizes=[128, 128, 128, 128],
        training_num=8, test_num=100, logdir="log", render=0.0,
        prioritized_replay=False, alpha=0.6, beta=0.4, resume=False,
        device="cuda" if torch.cuda.is_available() else "cpu", save_interval=4):
    env = gym.make(task)
    ...
    log_path = os.path.join(logdir, task, "c51")
